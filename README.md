...
Контракт HTTP для фронтенда: [docs/frontend.md](docs/frontend.md). Экраны, localStorage, what-if / alternatives / explore: [docs/frontend-ui.md](docs/frontend-ui.md). TypeScript-клиент (скопировать в UI): [docs/frontend-client.ts](docs/frontend-client.ts). Деплой Docker: [docs/docker.md](docs/docker.md).
...
Полный контракт для фронтенда (схемы, статусы, примеры, потоки UI, фикстуры, чего нет): **[docs/frontend.md](docs/frontend.md)**. Как собрать экраны без догадок: **[docs/frontend-ui.md](docs/frontend-ui.md)**.
